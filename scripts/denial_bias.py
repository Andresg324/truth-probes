...
for tag in TAGS:
    ...
    print(f"{tag:9s} {deny*100:>10.0f}% {affirm*100:>13.0f}% {deny-affirm:>+9.2f} (n={len(true_s)}/{len(false_s)})")
    report(tag, "denial_bias", "rates",
           {"deny_true": float(deny), "affirm_false": float(affirm), "asymmetry": float(deny - affirm)},
           n={"n_true": len(true_s), "n_false": len(false_s)})
